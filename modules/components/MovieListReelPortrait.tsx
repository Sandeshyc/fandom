...
import MovieCardReelSale from '@/modules/elements/MovieCardReelSale';
...
// Main Component
const MovieListReelPortrait: React.FC<MovieListProps> = ({ data, title, portrait, link, linkText, gradient = false, isBoxesLayout = false, marginTop=false }) => {
  ...
  const ReelContent = ()=> (<div className={` z-10 relative mt-[2vw] mb-[3vw] movieSlider ${portrait ? 'portrait': ""}`}>
    <div className="movieSliderInner">
      ...
      <div className="block lg:hidden">
        <div className='flex overflow-y-hidden overflow-x-auto mobileCardsSlide'>
          {newData?.map((movie, index) => (
            <MovieCardReelSale key={stableKeys[index]} data={movie} portrait={portrait} gradient={gradient} setRemovedItem={setRemovedItem}/>
          ))}
        </div>
      </div>
      <div className="hidden lg:block movieSliderReel">
        ...
        {...settings}>
          {newData?.map((movie, index) => (
            <MovieCardReelSale key={stableKeys[index]} data={movie} portrait={portrait} gradient={gradient} sliderRef={sliderRef} setRemovedItem={setRemovedItem}/>
          ))}
        </Slider>  
      </div> 
    </div>
...
export default MovieListReelPortrait;
