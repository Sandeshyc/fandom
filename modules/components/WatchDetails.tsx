...
const WatchDetails = (inputProps: dataProps) => {
  ...
  useEffect(() => {
    ...
    if (data?.allowed?.allowed && data?.allowed?.canPlay) {
      ...
      if (Array.isArray(data?.videoUrls) && data?.videoUrls.length > 0) {
        data?.videoUrls.map((item: any) => {
          if (
            !VideoURLs.HLS &&
            item?.label?.toString().toUpperCase() === "HLS"
          ) {
            VideoURLs.HLS = item?.url;
          }
          if (
            !VideoURLs.DASH &&
            item?.label?.toString().toUpperCase() === "DASH"
          ) {
    ...
    if (trailer === "true") {
      setIsTrailer(true);
      let trailerURL = data?.trailerVideo
        ? data?.trailerVideo
        : data?.trailerUrl;
      if (
        !trailerURL &&
  ...
  const captionURL =
    Array.isArray(data?.cc) && data.cc.length > 0 ? data?.cc : null;
  ...
  useEffect(() => {
    if (
      ...
      pcPinEnable &&
      pcPin &&
      Array.isArray(pcRoles) &&
      pcRoles.length > 0 &&
      movieContentRating &&
      pcRoles.includes(movieContentRating as never)
    ) {
      setPinMode(true);
    } else {
      setPinMode(false);
  ...
  useEffect(() => {
    if (
      ...
      data?.allowed?.canPlay &&
      data?.allowed?.bought &&
      !data?.allowed?.validityAccepted
    ) {
  ...
  return (
    <>
      {!pinMode && isMovieStartPopUp && (
        <div
          className="h-screen w-screen bg-black flex items-center relative"
          style={{
        ...
        >
          <div className="absolute top-0 left-0 w-full h-full bg-black/80 z-10 py-[150px] px-8 flex justify-center items-center">
            <div className="py-2 px-4 bg-black/90 w-[280px] sm:w-[420px] max-w-full flex justify-center flex-col">
              <Title tag="h3" size="xl" className="text-white text-center">
                Parental Control
              </Title>
              ...
              <div className="mt-4 justify-center items-center flex-col flex">
                {isPinSuccess && (
                  <p className="text-green-500 text-[14px]">
                    PIN Success, Please wait a moment...
                  </p>
                )}
                {isPinFail && (
                ...
                <LinkRoute
                  type="unset"
                  href="/myprofile"
                  className="text-[#fff]/90 text-[14px] py-1"
                >
                  Forgot PIN?
                </LinkRoute>
                <button
      ...
      {!pinMode && !isMovieStartPopUp && (
        ...
        >
          {(mouseActive || backMouseActive) && (
            <nav
              className={`fixed w-full p-4 z-50 top-1 flex flex-row items-center gap-4 lg:gap-8 bg-opacity-70 transition-opacity ease-in duration-700 ${
                backBtnActive ? "opacity-50" : "opacity-100"
              } videoPageNav`}
            ...
            >
              <ArrowLeftIcon
                onClick={handleBack}
                className={`w-8 md:w-12 text-white ${
                  backBtnActive ? "cursor-wait" : "cursor-pointer"
                } hover:opacity-80 transition border-2 border-blue-500 rounded-full p-1`}
              />
          ...
          <div
            className="channel_jwp_full h-[80vh] lg:h-screen w-screen"
            style={{
              backgroundImage: `url(${thumb})`,
              backgroundSize: "cover",
              backgroundPosition: "center",
