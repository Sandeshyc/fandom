...
const MembershipDetails = () => {
    ...
    return (
        <>
        {(isReady && !isLoading && Array.isArray(allowedItemLists) && allowedItemLists.length > 0) && (    
        <div className="px-4 md:px-12 mb-[2vw]">
            <div className="container mx-auto max-w-[996px]">
                <div className={`p-4 border border-[#C1C0C0] rounded-md bg-[#FFF] bg-opacity-[22%]`}>
                    ...
                    <div className={`text-[#11355E] py-4 rounded-md mt-4 flex justify-between flex-wrap ${
                            ...
                            <table className="w-full text-left paymentHistoryTable">
                                ...
                                <tbody className="text-sm">
                                    {allowedItemLists.map((item, index) => {
                                        return (
                                            <tr key={stableKeys[index]}
                                            className={`text-gray-900/70 block lg:table-row ${
                                              index % 2 === 0 ? "bg-white/20" : ""
                                            }`}>
                                                <td className={cellClass} data-label={"Plan Name"}>
                                                    {item?.purchase?.planName}
                                                </td>
                                                <td className={cellClass} data-label={"Member ID"}>
                                                    {item?.membership?.membershipId}
                                                </td>
                                                <td className={cellClass} data-label={"Purchase Date"}>
                                                    {convertESTtoLocalTime(item?.purchase?.purchaseDate as string)}
                                                </td>
                                                <td className={cellClass} data-label={"Start date"}>
                                                    {convertESTtoLocalTime(item?.header?.startDate as string)}
                                                </td>
