...
const BaseComponent = (props: any) => {
  ...
  if (queryMap[module.sourceType]) {
    ...
    const {
      ...
      error,
      data: gqData,
    } = useQuery(queryMap[module.sourceType], {
      variables: {
        input: {
          id: module.itemCode ?? module.source ?? "",
    ...
    if (error)
      return (
        <ErrorPopUp
          ...
          errorMsg={`GQL Error :${module?.sourceType as string} => ${
            module?.displayType as string
          } => ${module?.title as string} => ${error}`}
        />
      );
