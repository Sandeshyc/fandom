...
const EventCardReel: React.FC<MovieCardProps> = ({ data, portrait, gradient, sliderRef, setRemovedItem }) => {
    const router = useRouter();
    const [isAuthenticated, setIsAuthenticated] = useState(false);
    const { openModal, updateModal} = useMoviePopupStore();
    const [userId, setUserId] = useState('');
    const [isInWatchListTemp, setIsInWatchListTemp] = useState(data?.isInWatchList || false);
    ...
    return(
        ...
        className={`group bg-zinc-900 rounded-md relative border border-contentColor/10`} >
            <div className='flex flex-wrap flex-col sm:flex-row'>
                <div className='w-full h-auto sm:w-1/3 sm:bg-zinc-700 sm:aspect-[6/9] scale-105 cursor-pointer relative'
                onClick={redirectToDetails}>
                    <CardHeader header={data?.header} />
                    <img src={thumbURl} alt={title} className='w-1/2 sm:w-full sm:h-full  object-cover rounded-md bg-zinc-500 mx-auto' />
                    <div className='absolute z-30 bottom-0 right-0 w-full '>
                        <CardFooter footer={data?.footer} />
                    </div>
                </div>
                <div className='sm:w-2/3 p-4'>
                    <Title tag='h3' size='xl'>{title}</Title>                    
                    {(description)&&
                        <div className='mt-2 mb-4 text-white/60'>
                            <Text size='sm' clamp={3}>{description}</Text>
                        </div>
                    }
