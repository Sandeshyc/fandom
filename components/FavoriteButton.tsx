...
const FavoriteButton: React.FC<FavoriteButtonProps> = ({
  ...
  return (
    <button
      ...
      className={`cursor-pointer group/item w-8 h-8 ${
        isInLish ? "border-white" : "border-white/60"
      } border-2 rounded-full flex justify-center items-center transition active:opacity-65 hover:border-neutral-300 ${classes}`}
      style={style}
    >
      {(isListLoading)?(
        <RefreshOutlined
