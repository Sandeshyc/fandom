...
const ForgetPassword = () => {
  ...
  // Destructure the formik object
  const { errors:errors2, touched:touched2, values:values2, handleChange:handleChange2, handleSubmit:handleSubmit2 } = formik2;
  return (
    <AuthFrame authLoading={authLoading} pageHeading="Forget Password">
        ...
        <form onSubmit={handleSubmit2} method="POST">
          <div className="mb-4">
            ...
            <div className='mb-4 text-left mt-3'>
              ...
              {errors2.password && touched2.password && <span className='text-red-500 w-full text-xs'>{errors2.password}</span>}
            </div>
            <div className='mb-4 text-left'>
              ...
              {errors2.confirmPassword && touched2.confirmPassword && <span className='text-red-500 w-full text-xs'>{errors2.confirmPassword}</span>}
            </div>
            {(isUpdateSuccess) &&(
            ...
            {(otp?.length < 6 || isConfirmingOTP)?
                <span className='block h-[36px] py-1 text-[#fff] rounded-[50px] w-full transition bg-gray-400 cursor-not-allowed mt-2'>{(isConfirmingOTP)?'Loading...':'Update Password'}</span>
                :
                <button 
                type="submit"
      ...
      <p className="flex flex-wrap justify-center text-white mt-8">
        ...
        <Link href={'/login'} className="text-[#5F576F] text-[14px] cursor-pointer underline">
          Back Login
        </Link>
      </p>
    </AuthFrame>
